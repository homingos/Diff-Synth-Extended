...
from diffsynth.trainers.cached_latent_dataset import (
    CachedLatentDataset,
    CachedLatentDataLoader,
)
...
class CachedFeatureMergeTrainer(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        for i in range(batch_size):
            ...
            color_tensor = torch.tensor(soft_colors[i], device=self.device).view(
                3, 1, 1, 1
            )
            pred_rgb_soft = pred_rgb[i] * pred_alpha[i] + color_tensor * (
                1 - pred_alpha[i]
            )
            pred_rgb_soft_list.append(pred_rgb_soft)

            # Hard rendering: R_h = RGB if α > 0.5 else c
            color_tensor = torch.tensor(hard_colors[i], device=self.device).view(
                3, 1, 1, 1
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Feature Merge Block with cached latents"
    )
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="./models/train/feature_merge_cached",
    ...
    parser.add_argument(
        "--wandb_project",
        type=str,
        default="wan-alpha-feature-merge",
