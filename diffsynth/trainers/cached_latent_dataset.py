...
class CachedLatentDataset(Dataset):
    ...
    def get_batch(self, indices: List[int]) -> Dict[str, torch.Tensor]:
        ...
        if self.load_latents:
            ...
            output["alpha_latents"] = torch.stack(
                [d["alpha_latent"] for d in batch_data]
            )

        if self.load_tensors:
            output["rgb_tensors"] = torch.stack([d["rgb_tensor"] for d in batch_data])
            output["alpha_tensors"] = torch.stack(
                [d["alpha_tensor"] for d in batch_data]
            )
            output["hard_rgb_tensors"] = torch.stack(
                [d["hard_rgb_tensor"] for d in batch_data]
            )
            output["soft_rgb_tensors"] = torch.stack(
                [d["soft_rgb_tensor"] for d in batch_data]
            )
